...
- 站点统计存储：`.data/word-islands-stats.json`
...
- `WORD_ISLANDS_STATS_PATH` 不填时默认写入项目根目录 `.data/word-islands-stats.json`
...
- `/api/stats/track` - 记录一次访问，并在首次访问时写入长期 visitor cookie
- `/api/stats` - admin-only 统计接口，返回总访问、独立访客和注册人数
...
- 页面明明 build 成功，但浏览器还是拿旧的 `/_next/static/...` 文件

先不要立刻怀疑业务代码，按这个顺序排查：

1. 服务器当前 `.next` 里的首页 chunk 名称
2. `curl http://127.0.0.1:3000` 返回的首页 HTML 实际引用的 chunk 名称
3. `curl https://wordislands.cn` 返回的首页 HTML 实际引用的 chunk 名称

如果这三者不一致，问题就在：

- 运行中的服务没有切到新 build
- 或域名入口 / 浏览器仍在使用旧首页 HTML
